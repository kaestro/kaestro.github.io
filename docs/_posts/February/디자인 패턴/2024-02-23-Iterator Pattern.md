...
head first design patterns는 여러 가지 음식점을 통합해야하는 상황을 이야기합니다. 음식점의 종류에는 팬케이크 하우스, 디너 하우스, 카페가 있고 이들은 각각의 메뉴를 ArrayList, Array, HashMap과 같이 다른 형태로 관리하고 있습니다. 이런 상황에서 한 명의 웨이트리스에게 메뉴들을 읽는 다른 방법들을 다 숙지시키고 일을 맡기는 것은 비효율적이고 위험합니다.

이때 반복자 패턴을 통해 해결책으로 사용하는 것이 바로 반복자 객체입니다. 웨이트리스는 개별의 객체들을 순회하는 방법들에 대해 인지할 필요 없이 이를 캡슐화한 반복자 객체를 사용해서 각각의 객체들을 순회할 수 있게 됩니다.
...
1. **반복자 패턴**은 컬렉션의 내부 구조를 노출하지 않고 컬렉션의 모든 요소에 접근할 수 있게 하는 패턴입니다.
...
마을의 디너 하우스에서 사업 확장의 일환으로 팬케이크 하우스와 합병을 하기로 결정했습니다. 이 합병은 기존에 식사는 훌륭했지만 디저트에서 아쉬웠던 디너 하우스와 시너지 작용을 해서 더 많은 손님을 유치할 수 있으리라 기대했습니다.
...
위와 같은 상황을 개선하지 않으면 앞으로 더 많은 음식점을 통합하려는 목표를 가진 주인 입장에서는 심각한 걱정거리입니다. 여태까지 디자인 패턴을 적용하면서 이런 문제를 해결해왔듯이, 반복이 되면서 변하는 부분을 찾아내고, 이를 캡슐화해서 재사용할 수 있게 만들어야 합니다.
